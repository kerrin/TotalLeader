...
public class Square {
	...
	public Square(Player owner, short units, CoOrdinate coordinate) {
		type = TYPE.LAND;
		this.owner = owner;
		this.units = units;
	...
	public Square(TYPE type, Player owner, short units, CoOrdinate coordinate) {
		this.type = type;
		this.owner = owner;
		this.units = units;
		this.coordinate = coordinate;
	...
	public boolean makeLand(short units) {
		if(type != TYPE.SEA) return false;
		short cost = (short)Main.config.getInt(Config.KEY.BUILD_BRIDGE.getKey());
	...
	public void mouseClicked() {
		...
		if(Main.players[Main.currentPlayerIndex].getType() != Player.TYPE.PLAYER) return;
		
		selected = !selected;
		if(selected) {
			Main.eventManager.selected(coordinate);
		} else {
			Main.eventManager.deselected(coordinate);
		}
	}
	...
	private boolean buildLand(Square toSquare, short amount) {
		Logger.debug(Main.currentPlayerIndex+")Land Building with "+amount);
		short cost = (short)Main.config.getInt(Config.KEY.BUILD_BRIDGE.getKey());
	...
	public boolean attack(Square toSquare, short amount) {
		Logger.debug(Main.currentPlayerIndex+")Attack!");
		short defUnits = toSquare.getUnits();
		short attackUnits = amount;
		if(attackUnits > units) attackUnits = units;
		short baseDefenceUnits = (short)Main.config.getInt(Config.KEY.BASE_DEFENCE_UNITS.getKey());
	 ...
	 * @return
	 */
	public boolean startHasPlayerOverlap() {
		...
		for(int i = 0; i < 4; i++) {
			...
			Square square = Main.board.getBoard()[willBeMineX][willBeMineY];
			// Is this square owned by a player
			if(!square.getOwner().equals(Main.players[Main.nativePlayerIndex]) && 
					!square.getOwner().equals(Main.players[Main.seaPlayerIndex])) return true;
		}
		return false;
	}
}
