...
public class Player {
	 ...
	 */
	public Player(int playerIndex, TYPE type, String name, Color color, GameStatus gameStatus, Board board) {
	...
	public void recruitments() {
		recruits = ownSquareCount/gameStatus.config.getInt(Config.KEY.RECRUIT_SQUARES.getKey());
	}
	...
	public void score() {
		if(ownUnitsCount <= 0) {
			...
			score = ownSquareCount*gameStatus.config.getInt(Config.KEY.SQUARE_SCORE.getKey());
			score += ownUnitsCount*gameStatus.config.getInt(Config.KEY.UNIT_SCORE.getKey());
		}
		// Factor in the length of the game, if the game didn't go to full moves, scale it up
		score *= (gameStatus.config.getInt(Config.KEY.GAME_TURNS.getKey()) / (gameStatus.currentTurn*1.0));
